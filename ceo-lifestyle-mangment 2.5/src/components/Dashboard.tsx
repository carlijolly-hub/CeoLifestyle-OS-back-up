  ...
  return (
    <div className="space-y-10 animate-fade-in text-slate-800">
                        ...
                        {dashboardPromotions.slice(0, 5).map(promo => (
                          <div key={promo.client.id} className="p-3 bg-gradient-to-br from-slate-50 to-amber-50/30 rounded-2xl border border-slate-200/70 flex items-center justify-between gap-3">
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                const updatedRegister = approveClientPromotion(promo.client.id, promo.eligibleTier, tierRegister);
                                setTierRegister(updatedRegister);
                              }}
          ...
          <div className="bg-slate-900 text-white rounded-3xl p-6 shadow-lg border border-slate-800 space-y-4 text-left">
            ...
            {watchtowerIntelligence.length === 0 ? (
              ...
              <div className="space-y-2 max-h-[320px] overflow-y-auto pr-1">
                {watchtowerIntelligence.map((alert) => (
                  <div
                    key={alert.id}
      ...
      {selectedDashboardPromotion && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4 text-left">
      ...
      {showAddAspiringModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in" id="quick-add-aspiring-modal">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-200/80 space-y-5 relative text-left">
            <div className="flex items-center justify-between border-b pb-3 border-slate-100">
              <div className="flex items-center gap-2">
                <div className="p-2 bg-pink-50 rounded-xl border border-pink-100 text-pink-600">
                  <UserPlus className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-base font-extrabold text-slate-900">Add Aspiring Client</h3>
                  <p className="text-xs text-slate-500 font-medium">Quick prospect entry directly from Dashboard</p>
                </div>
              </div>
              <button 
                type="button"
                onClick={() => setShowAddAspiringModal(false)}
            ...
            <form onSubmit={handleSaveNewAspiring} className="space-y-4">
              {aspiringFormError && (
              ...
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  required
                  value={newAspiringData.name}
                  onChange={(e) => setNewAspiringData({ ...newAspiringData, name: e.target.value })}
              ...
              <div className="bg-slate-50/80 p-3.5 rounded-2xl border border-slate-200/70 space-y-3">
                <div className="flex items-center justify-between border-b pb-2 border-slate-200/60">
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
                    <Smartphone className="w-3 h-3 text-indigo-600" /> Contact Details
                  </span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <input
                      type="text"
                      value={newAspiringData.phoneNumber}
                      onChange={(e) => setNewAspiringData({ ...newAspiringData, phoneNumber: e.target.value })}
                      placeholder="e.g. +1 (876) 555-0192"
                      className="w-full bg-white border border-slate-200/70 rounded-xl px-3 py-2 text-xs font-bold text-slate-800 focus:border-indigo-500 focus:outline-hidden"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      type="email"
                      value={newAspiringData.email}
                      onChange={(e) => setNewAspiringData({ ...newAspiringData, email: e.target.value })}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                  <div className="space-y-1">
                    <label className="text-[10px] font-extrabold uppercase tracking-wider text-pink-600">
                      Instagram Username (optional)
                    </label>
                    <input
                      type="text"
                      value={newAspiringData.instagramUsername}
                      onChange={(e) => setNewAspiringData({ ...newAspiringData, instagramUsername: e.target.value })}
                      onBlur={() => {
                        if (newAspiringData.instagramUsername) {
                          setNewAspiringData({ ...newAspiringData, instagramUsername: formatInstagramUsername(newAspiringData.instagramUsername) });
                        }
                      }}
                      placeholder="e.g. @jordansdesigns"
                      className="w-full bg-white border border-pink-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-800 focus:border-pink-500 focus:outline-hidden"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <select
                      value={newAspiringData.preferredContactMethod}
                      onChange={(e) => setNewAspiringData({ ...newAspiringData, preferredContactMethod: e.target.value })}
                      className="w-full bg-white border border-indigo-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-800 focus:border-indigo-500 focus:outline-hidden"
                    >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">
                    Interest / Product Requested
                  </label>
                  <input
                    type="text"
                    value={newAspiringData.serviceInterestedIn}
                    onChange={(e) => setNewAspiringData({ ...newAspiringData, serviceInterestedIn: e.target.value })}
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="date"
                    value={newAspiringData.followUpDate}
                    onChange={(e) => setNewAspiringData({ ...newAspiringData, followUpDate: e.target.value })}
                    className="w-full bg-slate-50 border border-slate-200/70 rounded-xl px-3 py-2 text-xs font-bold text-slate-800 focus:bg-white focus:border-indigo-500 focus:outline-hidden"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  rows={2}
                  value={newAspiringData.notes}
                  onChange={(e) => setNewAspiringData({ ...newAspiringData, notes: e.target.value })}
                  placeholder="e.g. Inquired about bulk discount for 50 pieces..."
                  className="w-full bg-slate-50 border border-slate-200/70 rounded-xl px-3 py-2 text-xs font-medium text-slate-800 focus:bg-white focus:border-indigo-500 focus:outline-hidden resize-none"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 bg-gradient-to-r from-pink-600 to-indigo-600 hover:from-pink-500 hover:to-indigo-500 text-white font-extrabold text-xs rounded-xl shadow-xs transition-all cursor-pointer"
                >
                  Save Aspiring Client
                </button>
              </div>
            </form>
          </div>
